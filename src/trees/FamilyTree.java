...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Exception of index");
		...
		else
		{
			...
			if (parentNode == null) //If the parent node wasn't found, there must have been something wrong in the 
				...
				throw new TreeException("Error parentNode not found");
		}
		
		// Add child nodes to parentNode.
		for (String file: childrenArray) {//For each name in childrenArray, create a new node and add that node to parentNode.
			TreeNode curr = new TreeNode(file);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("Name 1 is null");//Throw a TreeException with a useful message
		TreeNode node2 = root.getNodeWithName(name2);	// node whose name is name2
		if (node2 == null)
			throw new TreeException("Name 2 is null"); //Throw TreeException with a useful message
